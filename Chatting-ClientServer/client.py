...
class MyTableWidget(QWidget):
    def __init__(self, parent):
        ...
        self.tab1 = QWidget()
        self.tab2 = QWidget()
        self.tabs.addTab(self.tab1, "Confirmation")
        ...
        #object, row, col, row span, col span
        ...
        self.messageRecords = QLabel("<font color=\"#865858\">Hi, Welcome to the chat room</font>", self)
        self.messageRecords.setStyleSheet("background-color: #f0e4d7;");
        ...
        self.emojiBtn1 = QPushButton("ᕦ(ò_óˇ)ᕤ")
        self.emojiBtn1.clicked.connect(functools.partial(self.send_emoji, "ᕦ(ò_óˇ)ᕤ"))
        self.emojiBtn2 = QPushButton("ก็ʕ•͡ᴥ•ʔ ก้")
        self.emojiBtn2.clicked.connect(functools.partial(self.send_emoji, "ก็ʕ•͡ᴥ•ʔ ก้"))
        self.emojiBtn3 = QPushButton("(｡◕∀◕｡)")
        self.emojiBtn3.clicked.connect(functools.partial(self.send_emoji, "(｡◕∀◕｡)"))
        self.emojiBtn4 = QPushButton("( ˘･з･)")
        self.emojiBtn4.clicked.connect(functools.partial(self.send_emoji, "( ˘･з･)"))
        self.emojiBtn5 = QPushButton("(ಠ_ಠ)")
        self.emojiBtn5.clicked.connect(functools.partial(self.send_emoji, "(ಠ_ಠ)"))
        self.emojiBtn6 = QPushButton("(〒︿〒)")
        self.emojiBtn6.clicked.connect(functools.partial(self.send_emoji, "(〒︿〒)"))
        self.emojiBtn7 = QPushButton("( •_•))")
        self.emojiBtn7.clicked.connect(functools.partial(self.send_emoji, "( •_•)"))
        ...
        emojiLayout.addWidget(self.emojiBtn5)
        emojiLayout.addWidget(self.emojiBtn6)
        emojiLayout.addWidget(self.emojiBtn7)
    ...
    def enter_line(self):
        ...
        if self.sendTo != self.sendComboBox.currentText():
            #if it is send private check if the person still there, if not then clear
            self.message_display_append("Failed to deliver chat, the person have left the chat")
            self.lineEdit.clear()
            return
    ...
    def send_emoji(self, emoji):
        #assure the person still in rooom before send out
        if self.sendTo != self.sendComboBox.currentText():
            self.message_display_append("The person have left the chat")
            return
        if self.sendTo != "ALL":#private message, send to myself first
            #this is a trick leverage the server sending back a copy to myself
    ...
    def connect_server(self):
        ...
        if port == "" or not port.isnumeric():
            self.portLineEdit.setText("8080")
            self.connStatus.setText("Status :"+"Sorry port invalid")
            return 
        else:
            port = int(port)
        try:
